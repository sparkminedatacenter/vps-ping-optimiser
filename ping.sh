...
spinner() {
    ...
    while kill -0 "$pid" 2>/dev/null; do
        for c in "${SPIN_CHARS[@]}"; do
            echo -ne "\r${YELLOW}⟳ applying network optimisation and stabilizing latency ${c}${RESET}"
            sleep $delay
        done
    done
    echo -e "\r${GREEN}✔ Optimization completed!            ${RESET}\n"
}

# Prompt for ping optimizer
read -p "Do you want to run ping optimizer? [Y/n] " answer
case "$answer" in
...
install_dependencies() {
    apt-get update -qq
    apt-get install -y -qq ethtool irqbalance
}
install_dependencies & deps_pid=$!
spinner "$deps_pid"
...
EOF
    ...
    # Disable NIC offloading (update interface name if not eth0)
